...
{   
    public:
        ...
        ~KLruCache() override=default;  //析构函数

        //添加缓存
        void put(Key key, Value value) override //override表示覆盖基类当中的虚函数
        {
            if(capacity_ <= 0) 
            return; 
            ...
            if(it!=NodeMap_.end())
            {
                //如果在当前容器当中，则更新value，并调用get方法，代表该数据刚被访问
    ...
    private:
        ...
        void updateExistingNode(NodePtr ndoe,const Value& value)
        {
            node->setValue(value); 
        ...
        {
            if(!node->prev_.expired()&&node->next_) //expired() 是 std::weak_ptr 提供的一个成员函数，
                                                    //专门用来判断这个弱引用是不是已经失效了。
            {
                auto prev=node->prev_.lock(); //lock() 是 std::weak_ptr 的大杀器，
                prev->next_=node->next_;                          //用来把它临时升级成一个能用的 shared_ptr，从而访问对象！
                node->next_->prev_=prev;
                node->next_=nullptr; //清空next_指针，彻底断开节点与链表的联系
        ...
        //从尾部插入节点    
        void insertNdoe(NodePtr node)
            {
                node->next_=dummyTail_;
                node->prev_=dummyTail_->prev_;
                dummyHead_->prev_->next_=node; //使用lock()获取shared_ptr
...
class KLruKCache:public KLruCache<Key,Value>
{
    ...
    {}
        ...
        Value get(Key key)
        {
            ...
            if(historyCount>=k_)
            {
                ...
                if(it!=historyValueMap_.end()){
                    ...
                    //添加到主缓存
                    KLruCache<Key,Value>::put(key,storeValue);
                    return storeValue;
                }
                //没有历史记录，无法添加到缓存，返回默认值
            }

            //数据不在主缓存且不满足访问次数的添加条件，返回默认值
            return value;
        }
        ...
        void put(Key key,Value value)
        {
            ...
            size_t historyCount=historyList_->get(key);
            historyCount++;
            KLruCache<Key,Value>::put(key,historyCount);
